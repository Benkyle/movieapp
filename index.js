...
const setGenres = () => {
  ...
  const genreSelector = document.querySelector(".dropdown-content");
  ...
  fetch(url, options)
    ...
    .then((json) => {
      ...
      genres.forEach((genre) => {
        ...
        tag.addEventListener("click", () => handleGenreClick(genre.id));
        genreSelector.appendChild(link);
        tagsSelector.appendChild(tag);
      });
    })
...
function showMovies(data) {
  ...
  data.forEach((movie, index) => {
    ...
    let color = "green";
    if (movie_rating < 5) color = "red";
    else if(movie_rating <= 6) color = "orange"
